...
export const testsApi = {
  getBySuite: (suiteId: string) => api.get<Test[]>(`/suites/${suiteId}/tests`).then((r) => r.data),
  getOne: (id: string) => api.get<Test & { steps: TestStep[] }>(`/tests/${id}`).then((r) => r.data),
  ...
  getVersions: (id: string) => api.get<TestVersion[]>(`/tests/${id}/versions`).then((r) => r.data),
  createStep: (
    testId: string,
    dto: { action: string; selector?: string; value?: string; description?: string; order: number },
  ...
  deleteStep: (testId: string, stepId: string) => api.delete(`/tests/${testId}/steps/${stepId}`),
