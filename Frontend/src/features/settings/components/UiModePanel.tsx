...
export function UiModePanel() {
  ...
  return (
    <div className="max-w-2xl">
      <h2 className="mb-1 text-sm font-semibold text-gray-900">Cómo quieres ver la plataforma</h2>
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`h-4 w-4 shrink-0 rounded-full border-4 ${
                    activo ? 'border-indigo-600 bg-white' : 'border-gray-300 bg-white'
                  }`}
                />
                <span className="text-sm font-medium text-gray-900">{o.titulo}</span>
                {activo && <span className="text-xs text-indigo-600">· activo</span>}
              </div>
              <p className="ml-6 mt-1 text-sm text-gray-600">{o.texto}</p>
            </button>
          );
        })}
      </div>
