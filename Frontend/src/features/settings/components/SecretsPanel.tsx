  ...
  return (
    <div>
      ...
      <div className="rounded-xl border border-tinta-300 bg-tinta-50 overflow-hidden">
        {secrets.length === 0 && (
          <p className="p-6 text-center text-sm text-tinta-500">
        ...
        {secrets.map((s, i) => (
          ...
          >
            <div className="flex-1">
              <p className="text-sm font-medium text-tinta-800 font-mono">{s.name}</p>
              {s.description && <p className="text-xs text-tinta-500">{s.description}</p>}
            </div>
        ...
        <form onSubmit={(e) => void createSecret(e)} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-tinta-300 px-3 py-2 text-sm font-mono focus:border-oro-500 focus:outline-none"
              placeholder="API_KEY"
            />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-tinta-700">Tipo</label>
            <select
              value={form.type}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-tinta-700">Valor</label>
            <input
              type="password"
              required
              value={form.value}
