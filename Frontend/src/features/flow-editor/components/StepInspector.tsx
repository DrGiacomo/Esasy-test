...
export function StepInspector({ step, onChange }: Props) {
  if (!step) {
    return (
      <div className="flex w-64 items-center justify-center border-l border-linea bg-superficie p-6">
        <p className="text-center text-sm text-texto-tenue">Selecciona un paso para editarlo</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-64 border-l border-linea bg-superficie p-4">
      <p className="mb-4 text-sm font-semibold text-texto">Paso: {step.action}</p>
      <div className="space-y-3">
        {fields.map(({ key, label, placeholder }) => (
          <div key={key}>
            <label className="mb-1 block text-xs font-medium text-texto-tenue">{label}</label>
            <input
              value={String(step[key] ?? '')}
              onChange={(e) => onChange({ [key]: e.target.value })}
              placeholder={placeholder}
              className="w-full rounded-lg border border-linea px-2.5 py-1.5 text-sm focus:border-oro-500 focus:outline-none focus:ring-1 focus:ring-oro-300"
            />
          </div>
        ))}
      </div>
