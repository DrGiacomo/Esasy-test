...
export default function ReportViewerPage() {
  ...
  useEffect(() => {
    ...
    api.get<Report>(`/reports/${executionId}`).then((r) => setReport(r.data)).finally(() => setLoading(false));
  }, [executionId]);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500">{report.project.name} — {report.project.baseUrl}</p>
      </div>

      <ReportSummary summary={report.summary} status={report.status} startedAt={report.startedAt} completedAt={report.completedAt} />

      <div className="mt-6 space-y-3">
        {report.results.map((r) => <TestResultCard key={r.id} result={r} />)}
      </div>
    </div>
  );
}
