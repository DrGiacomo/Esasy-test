...
function stepLabel(step: RecordingStep): string {
  switch (step.type) {
    ...
    case 'click':    return step.selector ? step.selector : `(${step.x}, ${step.y})`;
    case 'dblclick': return step.selector ? step.selector : `(${step.x}, ${step.y})`;
    case 'fill':     return `"${step.value}" en ${step.selector}`;
    case 'type':     return `"${step.value}"`;
    case 'press':    return step.key ?? '';
    case 'select':   return `"${step.value}"`;
...
function stepColor(type: string): string {
  const map: Record<string, string> = {
    navigate: 'text-blue-400', click: 'text-green-400', dblclick: 'text-green-300',
    fill: 'text-yellow-400', type: 'text-yellow-300', press: 'text-purple-400',
    select: 'text-orange-400', hover: 'text-gray-400',
  };
...
function ConvertModal({ rec, onClose, onConverted }: {
  rec: Recording;
  onClose: () => void;
  ...
  useState(() => { if (selectedProjectId) loadSuites(selectedProjectId); });
    ...
    <form onSubmit={(e) => void handleConvert(e)} className="space-y-4">
      ...
      <div>
        ...
        {selectedProjectId && !loadingSuites && suites.length === 0 && (
          <p className="mt-1 text-xs text-amber-600">Este proyecto no tiene suites. Crea una en la página del proyecto primero.</p>
        )}
      </div>
      <p className="text-xs text-gray-500">Se crearán {rec.steps.length} pasos a partir de esta grabación.</p>
  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-gray-800 overflow-hidden">
      <div className="flex items-center gap-4 px-4 py-3">
        <button onClick={() => setOpen(o => !o)} className="flex flex-1 items-center gap-4 text-left min-w-0">
          ...
          <div className="flex items-center gap-4 shrink-0 text-xs text-gray-400">
            <span className="flex items-center gap-1"><Clock size={12} />{formatDuration(rec.startedAt, rec.stoppedAt)}</span>
            <span className="flex items-center gap-1"><Globe size={12} />{rec.steps.length} pasos</span>
      ...
      <Modal open={converting} title="Convertir grabación en test" onClose={() => setConverting(false)}>
        <ConvertModal rec={rec} onClose={() => setConverting(false)} onConverted={handleConverted} />
      </Modal>
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      {recordings.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 py-16 text-gray-500">
          ...
          <p className="text-xs text-gray-600">Inicia una sesión en el Grabador y haz click en Detener para guardar.</p>
        </div>
      ) : (
