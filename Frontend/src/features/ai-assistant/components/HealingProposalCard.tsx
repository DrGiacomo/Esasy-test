...
interface Props { proposal: SelectorHealingLog; onResolved: () => void }
...
export function HealingProposalCard({ proposal, onResolved }: Props) {
  ...
  async function approve() {
    setLoading(true);
    try { await api.post(`/ai/heal/${proposal.id}/approve`); onResolved(); }
    finally { setLoading(false); }
  }

  async function reject() {
    setLoading(true);
    try { await api.post(`/ai/heal/${proposal.id}/reject`, { rejectionReason: 'Rechazado manualmente' }); onResolved(); }
    finally { setLoading(false); }
  }
    ...
    <div className="rounded-xl border border-yellow-200 bg-yellow-50 p-4">
      ...
      <div className="mb-3 grid grid-cols-2 gap-2 text-xs font-mono">
        <div className="rounded bg-red-50 px-2 py-1 text-red-600">
          <span className="font-medium text-red-500">Antes: </span>{proposal.originalSelector}
        </div>
        <div className="rounded bg-emerald-50 px-2 py-1 text-emerald-600">
          <span className="font-medium text-emerald-500">Propuesta: </span>{proposal.proposedSelector}
        </div>
      </div>
